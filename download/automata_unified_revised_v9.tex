...
The full-KL retention gap\footnote{Here and throughout, ``retention gap'' is this manuscript's state-compression cost --- the stationary Kullback--Leibler price of lumping predictive states into at most $M$ blocks --- instantiated as the $\RetKL(M)$ of Definition~\ref{def:full-kl-retention}, its controlled relative $\RetKLc$ of Definition~\ref{def:controlled-full-kl}, and the quadratic surrogate $\RetQuad$ of Definition~\ref{def:gaussian-quadratic}.  It is unrelated to the same phrase in the thermodynamics-of-learning literature, where a retention gap is a value-side quantity $L_{\mathrm{gen}}$ for finite-state learning devices under task-distribution shift \cite{sudo2026}.} is
\[
\RetKL(M)
=
\min_{\substack{
...
quotient-typed \emph{symbolic} grounding
...
block-local problem but is not the discrete right-congruence Price of
...
lower bounds are explicit forcing constructions rather than Littlestone
transfers, and agnostic regret is $\Theta(\sqrt{TM\log M})$.
...
its Myhill--Nerode index is $1$.  Every cost used in this manuscript --- KL divergence,
...
The Myhill--Nerode equivalence is always a right congruence.  Indeed, if
...
--- $H_\nu$, $\bm{\Sigma}_\pi$, $N_\Lambda$ --- with different effective rank
...
$\bm{\Sigma}_\pi$.  No tightness claim is made for full KL.
\end{remark}
...
-
\frac{r^2}{2}
\operatorname{tr}(\bm{\Sigma}_\pi-B_\phi)
\right|
\le
C r^3,
\]
...
\RetKL(\phi)
\ \ge\
\sum_{i\ge M}\lambda_i(\bm{\Sigma}_p),
\]
and consequently
\[
\RetKL(M)
...
hypothesis is used, and $\bm{\Sigma}_p$ is the covariance of the predictive
...
The usual variance decomposition gives $\bm{\Sigma}_p=W_\phi+B_\phi$ with
...
$\bm{\Sigma}_p$ and $B_\phi$ are positive semidefinite and symmetric, so Ky Fan's
maximum principle \cite{kyfan1951} applies to eigenvalues and gives
\[
\tr(B_\phi)\ \le\ \sum_{i=1}^{M-1}\lambda_i(\bm{\Sigma}_p).
\]
Therefore
\[
\tr(W_\phi)=\tr(\bm{\Sigma}_p)-\tr(B_\phi)\ \ge\ \sum_{i\ge M}\lambda_i(\bm{\Sigma}_p),
\]
...
and take $M=1$.  Then
\[
\sum_{i\ge1}\lambda_i(\bm{\Sigma}_p)=\tr(\bm{\Sigma}_p)=2\varepsilon^2,
\qquad
\RetKL(1)=2\varepsilon^2+O(\varepsilon^4),
\]
so
\[
\frac{\RetKL(1)}{\sum_{i\ge1}\lambda_i(\bm{\Sigma}_p)}
\ \longrightarrow\ 1
\qquad(\varepsilon\downarrow0).
\]
Consequently no constant $c>1$ satisfies
$\RetKL(M)\ge c\sum_{i\ge M}\lambda_i(\bm{\Sigma}_p)$ for all instances.
\end{proposition}
...
Theorem~\ref{thm:global-kl-simplex} is stated in the covariance $\bm{\Sigma}_p$ of
the predictive \emph{probability vectors}.  The Fisher covariance
$\bm{\Sigma}_\pi$ of Theorem~\ref{thm:local-full-kl} is a different object, computed
...
$\bm{\Sigma}_F=I(\eta_{\bar p})^{1/2}\bm{\Sigma}_\eta I(\eta_{\bar p})^{1/2}$ is one such
...
\[
\bm{\Sigma}_\eta=\sum_s\pi_s(\eta_s-\bar\eta)(\eta_s-\bar\eta)^{\!\top}.
\]
Then
\[
\RetKL(M)\ \ge\ \frac{m_K}{2}\sum_{i\ge M}\lambda_i(\bm{\Sigma}_\eta).
\]
\end{theorem}
...
$\bm{\Sigma}_\eta=W_\phi+B_\phi$ with $\rank(B_\phi)\le M-1$ and Ky Fan's principle
\cite{kyfan1951} give $\tr(W_\phi)\ge\sum_{i\ge M}\lambda_i(\bm{\Sigma}_\eta)$,
...
\]
Then, with $\bm{\Sigma}_\pi$ the Fisher covariance of
Theorem~\ref{thm:local-full-kl} computed in the natural parameter,
\[
\RetKL(1)=\Theta(\varepsilon),
\qquad
\tr(\bm{\Sigma}_\pi)=\Theta(\varepsilon),
\qquad
\frac{\RetKL(1)}{\tr(\bm{\Sigma}_\pi)}
\longrightarrow
...
and let $\bm{\Sigma}_\eta$ be their $\pi$-covariance in that chart; for the
Fisher-weighted form fix in addition the reference point $\eta_{\bar p}$ of the
stationary mixture and set
$\bm{\Sigma}_F=I(\eta_{\bar p})^{1/2}\bm{\Sigma}_\eta I(\eta_{\bar p})^{1/2}$.

There is \emph{no} universal constant $c>0$, independent of the instance, with
\[
\RetKL(M)\ \ge\ c\sum_{i\ge M}\lambda_i(\bm{\Sigma}_\eta)
\qquad\text{or}\qquad
\RetKL(M)\ \ge\ c\sum_{i\ge M}\lambda_i(\bm{\Sigma}_F)
\]
...
\[
\bm{\Sigma}_\eta=L_\varepsilon^2\ \longrightarrow\ \infty
\qquad(\varepsilon\downarrow0).
\]

\emph{Conclusion for the unweighted form.}  Therefore
\[
\frac{\RetKL(1)}{\lambda_1(\bm{\Sigma}_\eta)}
\ \le\
...
Hence $\bm{\Sigma}_F=\tfrac14L_\varepsilon^2\to\infty$ as well, and
$\RetKL(1)/\lambda_1(\bm{\Sigma}_F)\le 4\log2/L_\varepsilon^2\to0$.  Weighting by the
...
\lambda_i(\bm{\Sigma}_\eta)$ with $m_K=\inf_{\eta\in K}\lambda_{\min}(\nabla^2A(\eta))$
...
block count admits at most $M-1$ increases.  Seventeen statements are checked
...
$\bm{\Sigma}_p=W_\phi+B_\phi$, $1.7\times10^{-16}$ for the Ky Fan bound
$\tr(B_\phi)\le\sum_{i\le M-1}\lambda_i$, and $7.5\times10^{-17}$ for the
composite conclusion $\RetKL(\phi)\ge\sum_{i\ge M}\lambda_i(\bm{\Sigma}_p)$.  The
...
For finite $\mathcal O$, let $\bm{\Sigma}_p^{x}$ denote the covariance of the
predictive probability vectors $\{p_s^x\}$ under the \emph{conditional} weights
$\{\pi_s^x\}$.  Then
\[
\RetKLg(M)
\ \ge\
\sum_xp(x)\sum_{i\ge M}\lambda_i\bigl(\bm{\Sigma}_p^{x}\bigr).
\]
\end{corollary}
...
\[
\bm{\Sigma}_\pi^x=\sum_s\pi_s(y_s^x-\bar y^x)(y_s^x-\bar y^x)^{\!\top},
\qquad
...
\RetQuadc(M)
\ \ge\
\frac12\sum_xp(x)\sum_{i\ge M}\lambda_i(\bm{\Sigma}_\pi^x).
\]
\end{corollary}
...
$\tfrac12\sum_{i\ge M}\lambda_i(\bm{\Sigma}_\pi^x)$, the effective rank of the
...
$\bm{\Sigma}_p^x$ their $\pi$-covariance on fiber $x$.  Then, globally and with no
regularity hypothesis,
\[
\RetKLc(M)
\ \ge\
\sum_xp(x)\sum_{i\ge M}\lambda_i(\bm{\Sigma}_p^x),
\]
...
\RetKLc(M)
\ \ge\
\frac12\Bigl(\min_xm_{K}^{x}\Bigr)\sum_xp(x)\sum_{i\ge M}\lambda_i(\bm{\Sigma}_\eta^x),
\]
where $\bm{\Sigma}_\eta^x$ is the $\pi$-covariance of the natural parameters on
fiber $x$.
\end{corollary}
...
For~(iii), write $w_s(b)=\pi_sP_s(b)$.  The statewise modal term
...
reverses the inequality to $D(\phi)\le D(\psi)$.  The feasible
...
\lambda_i(\bm{\Sigma}_\pi),
\]
not $\sum_{i>M}\lambda_i(\bm{\Sigma}_\pi)$.  A check using the latter does
...
$\bm{\Sigma}_\pi\succeq0$ be the stationary Fisher covariance.  For $M\ge1$,
\[
\RetQuad(M)
\ge
\frac12
...
\bar y=\sum_s\pi_sy_s,
\qquad
\bm{\Sigma}_\pi=\sum_s\pi_s(y_s-\bar y)\otimes(y_s-\bar y).
\]
...
\frac12\tr(\bm{\Sigma}_\pi-B),
\]
the last step being the ANOVA identity $\bm{\Sigma}_\pi=W+B$ with $W\succeq0$ the
within-cluster covariance.

Because $\bm{\Sigma}_\pi-B=W\succeq0$, its trace coincides with its nuclear norm,
\[
\tr(\bm{\Sigma}_\pi-B)=\norm{\bm{\Sigma}_\pi-B}_{\Sp{1}},
\]
...
\norm{\bm{\Sigma}_\pi-B}_{\Sp{1}}
\ \ge\
\min_{\rank P\le M-1}\norm{\bm{\Sigma}_\pi-P}_{\Sp{1}}
=
\sum_{i>M-1}\lambda_i(\bm{\Sigma}_\pi)
=
\sum_{i\ge M}\lambda_i(\bm{\Sigma}_\pi).
\]
Hence $E\ge\frac12\sum_{i\ge M}\lambda_i(\bm{\Sigma}_\pi)$, and taking the infimum
over feasible quotients gives the stated converse, with effective rank $M-1$
and $\Phi^{(1)}_{M-1}(\bm{\Sigma}_\pi)=\sum_{i\ge M}\lambda_i(\bm{\Sigma}_\pi)$.
\end{proof}
...
\]
The ANOVA decomposition gives $\bm{\Sigma}_\pi=W_{\mathcal C}+B_{\mathcal C}$ with
$W_{\mathcal C}\succeq0$ the within-cluster covariance, whence
\[
0\preceq B_{\mathcal C}\preceq\bm{\Sigma}_\pi .
\]
...
\[
\tr(B_{\mathcal C})\ \le\ \KyF{\bm{\Sigma}_\pi}{M-1}.
\]
The nonzero spectra of $\bm{\Sigma}_\pi$ and $G$ coincide, since $G$ is the Gram
matrix of the same weighted centred vectors, so
$\KyF{\bm{\Sigma}_\pi}{M-1}=\KyF{G}{M-1}$ and
\[
...
feature covariance $\bm{\Sigma}_\pi$ has the same nonzero spectrum and may be used
for the free term, but pinching $\bm{\Sigma}_\pi$ by feature-coordinate blocks is
...
operators $H_\nu$, $\bm{\Sigma}_\pi$, $N_\Lambda$, the rank budgets $M$, $M-1$, $M$
...
Remark~\ref{rem:computational-conventions} --- the worst-case state-identification cost never
...
exactly $3$ and $6$ respectively, the latter realized by $3072$ machines.  An explicit witness
at $M=4$ is
\[
...
$n=d+1$ fails for $d=2$, by a discriminant obstruction.  What no choice of scheme can avoid is
