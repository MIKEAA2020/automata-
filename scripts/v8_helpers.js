// v8_helpers.js — shared helpers for the v8 revision report.
// Cloned from the validated house generator pattern (cvc_helpers.js), which
...
const MONO_FONT = { ascii: "Consolas", eastAsia: "SimSun" };
...
function mono(text, opts = {}) {
  return new TextRun({ text, size: 19, color: P.body, font: MONO_FONT, ...opts });
}
...
function quoteBlock(text) {
  return new Paragraph({
    ...
    children: [new TextRun({ text, size: 20, color: P.secondary, font: EN_FONT, italics: true })],
  });
}
function cell(text, opts = {}) {
  const { bold = false, fill = null, w = 20, size = 20, color = P.body, font = EN_FONT } = opts;
...
module.exports = { P, buildCoverR1, h1, h2, body, bodyRuns, run, mono, bullet,
