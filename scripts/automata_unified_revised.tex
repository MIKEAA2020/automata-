...
\[
\mu(u)=(1-\beta)\beta^{|u|}\Pr[\text{prefix }u],
\qquad \beta\in(0,1),
\]
...
\]
Moreover $\ComRD(M)=0$ if and only if $M\ge\kappa_{\mathrm{obs}}(F^{(1)},\mu)$,
where $F^{(1)}(u)=F(u)_{|u|}$ for $u\ne\varepsilon$ and
$\kappa_{\mathrm{obs}}$ is the observable support index of
Definition~\ref{def:observable-support-index} instantiated at the one-step
output object $F^{(1)}$ with the $\{0,\infty\}$-valued equality cost; this
threshold depends on $\mu$ only through its support and is in general
strictly smaller than $\kappa_{\det}(F)$ of Theorem~\ref{thm:commitment-spec}.
\end{theorem}
...
$\mu$-almost surely determined by $([u_{1:|u|-1}]_\sim,u_{|u|})$; this is
exactly the condition, in Definition~\ref{def:observable-support-index}, that
$\sim$ admits a representative exact for $F^{(1)}$ on a set of full
$\mu$-measure.  Hence the minimum index of such a $\sim$ is
$\kappa_{\mathrm{obs}}(F^{(1)},\mu)$ by definition, and $\ComRD(M)=0$ exactly
for $M\ge\kappa_{\mathrm{obs}}(F^{(1)},\mu)$.  The inequality
$\kappa_{\mathrm{obs}}(F^{(1)},\mu)\le\kappa_{\det}(F)$ holds because the full
Myhill--Nerode congruence $\sim_F$ is in particular exact for the coarser
one-step object $F^{(1)}$ on all of $\mathcal I^*$; strictness occurs
whenever distinct residuals $F_u\ne F_v$ nonetheless agree on their very next
symbol for $\mu$-almost every continuation.
\end{proof}
...
$M=\kappa_{\mathrm{obs}}(F^{(1)},\mu)$; Remark~\ref{rem:com-rd-scope} shows
