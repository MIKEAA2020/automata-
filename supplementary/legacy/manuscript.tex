...
Grounding uses linear finite-rank Hankel realizations; exact symbolic
deterministic grounding remains open.  Task theories use stationary
...
alphabet, $|\Sigma|=1$, equality follows from Adamjan--Arov--Krein once a
shift-intertwining Hardy-space embedding is supplied.  For $|\Sigma|>1$ the
free monoid carries no shift of multiplicity one, the scalar theorem does not
apply, and equality remains conditional on a Nehari/AAK-type finite-rank
approximation theorem for the multi-shift setting, which is not established
here.
...
$\Theta_{|\mathcal O|}(\log M)$ mistakes --- the halving upper bound is matched
...
conflated.  The framework separates shared finite-state syntax from
regime-specific analysis and identifies the remaining open problem of exact
...
Exact symbolic deterministic grounding remains open.
\end{enumerate}

The separation distinguishes deterministic machines, stochastic predictors,
and linear realizations.  The common finite-state structure is the index of a
right congruence on histories.
...
aggregation, where
$\mu(u)=(1-\beta)\beta^{|u|}\Pr[\text{prefix }u]$ is a probability measure on
...
beyond Lean's standard three, namely propositional extensionality, the axiom of
choice, and quotient soundness.  The sources, a build script and the axiom
...
bounds.  The exponent correspondence identifies commitment with the
Booleanized symmetrized $\alpha=0$ support vertex, not with raw one-sided
R\'enyi order $0$ and not with ordinary Schatten rank.
...
Let
\[
F:\mathcal I^{\mathbb N}\to\mathcal O^{\mathbb N}
\]
be a deterministic specification.  For each finite input history $u$, define
the residual
\[
F_u(w)=F(uw)_{|u|+1:|u|+|w|}.
\]
The residual $F_u$ is the continuation behavior required after the history $u$.
...
\[
\Com(M)=\Valt(\infty)-\Vsim(M).
\]
It is an \textbf{observation-and-memory gap}: a positive value combines the
...
intrinsic lower bounds for deterministic approximation: an exponential
...
attained; for compact $H_\nu$ the optimal Hankel approximant of rank at most
$M$ is moreover unique \cite{aak1971}.  The two descriptions of the feasible
...
symbolic approximation is hard.  The following theorem gives an exponential
separation between stochastic linear rank and deterministic state complexity.
...
\begin{remark}[Scope of the Exponential Gap]
...
For every deterministic Mealy machine $\mathcal A$,
\[
\sup_{x\in\mathcal I^{\mathbb N}}
d_{\mathrm{KR},\gamma}
\bigl(
...
If the channel is non-degenerate, there exists an input sequence $x$ along
which the expected discounted stochastic spread is positive, so
$\sigma_\gamma(\nu)>0$.
\end{proof}
...
Let $\phi$ be any state compression and let $\mathcal A_\phi$ be the machine
that occupies block $\phi(S_t)$ and emits $\hat b_{\phi(S_t)}$.  Then the
stationary one-step error decomposes as
\[
...
$D(\phi)\le D(\psi)$ need not hold in general, but
$\min_{|\mathcal K|\le M}D(\phi)$ is nonincreasing in $M$, the feasible sets
being nested.
\end{enumerate}
\end{proposition}
...
$\hat b_{\{s\}}=\argmax_bP_s(b)$, so every bracket is zero.  Claim~(iii) is the
nesting of feasible sets.
\end{proof}
...
Remark~\ref{rem:tracking-deficit-scope}.  Consequently the grounding
compression problem is never harder than the retention one in the sense that
its zero set is larger, and the two thresholds coincide only when distinct
predictive laws have distinct modes.  No inequality between the two \emph{gap
values} follows, the costs being incomparable; this is the Independence
Theorem~\ref{thm:independence} in the present instance.
\end{remark}
...
If one insists on a dimension-free modulus, the linear-measurement route fails
for Hankel embeddings and a different mechanism is needed; this is the content
of the open problem on domination bridges.  If instead a budget-dependent
modulus $c_p=c_p(M)$ is admitted, the template still applies but the converse it
yields is weaker by that factor, and the weakening must be carried explicitly
into every downstream tail bound rather than absorbed into a constant.
...
\]
and modulus $c_p>0$ if every budget-$M$ approximant $\widehat\delta$ is
assigned a response operator $A_{\widehat\delta}$ such that:
\begin{enumerate}[label=(\alph*)]
...
$A_\delta$, effective rank budget $r_{\mathbb T}(M)$, and modulus $c_p$.  Then
\[
\Delta_{\mathbb T}(M)
\ge
\frac1{c_p}
...
The restriction to feasible triples is not dispensable.  If
...
(Section~\ref{sec:openproblems}).  Those regimes are stochastic, and by
...
For every $\vec\varepsilon=(\varepsilon_1,\dots,\varepsilon_m)$,
\[
...
\]
provided each $M_i(\varepsilon_i)$ is finite and attained by some congruence
and representative map.  Without attainment the upper bound holds with each
$\varepsilon_i$ replaced by $\varepsilon_i+\eta_i$, for arbitrary
$\eta_i>0$.
\end{theorem}
...
minimal skeleton costs $\Theta_{|\mathcal O|}(\log M)$ mistakes, so no
...
state-identification mistake complexity is $O_{|\mathcal O|}(\log M)$, so a
...
\item Stateless-game exact corollary $\Com(M)=\alpha/(1-\gamma)$.
...
\item Stochasticity floor for deterministic Mealy grounding.
...
\item \textbf{Exact symbolic deterministic grounding gap.}
Determine
\[
\Delta_{\mathrm{grd}}^{\mathrm{symb}}(M)
\]
for deterministic Mealy machines, and relate it to the unrestricted linear
finite-rank Hankel gap, stochasticity floors, and exponential determinism
gaps.
...
The unrestricted full-KL decision problem is: given $\{P_s\}$, $\{\pi_s\}$,
lumpability constraints and $\tau$, decide whether
\[
\min_{\phi}
...
\sigma_{M+1}(H_\nu),
\]
with equality only under additional structural hypotheses.  Exact symbolic
deterministic grounding remains open.  AAK is used only under separate
Hardy-space embeddings.
...
instance-dependent.  See Section~\ref{sec:openproblems}.
...
a known minimal skeleton costs only $O_{|\mathcal O|}(\log M)$ mistakes
