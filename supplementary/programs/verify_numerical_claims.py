#!/usr/bin/env python3
"""Numerical verification suite for the manuscript's computational claims.

Supplementary package component: reproduces, independently, every recomputable
computational observation quoted in the manuscript, under the conventions of
the remark on Computational Conventions (exact rational arithmetic where stated;
double precision only where the manuscript itself uses it, with the
near-degenerate comparisons flagged there).

Check -> manuscript statement map:
  1. Counter family C_M (remark on the refinement-extremal scope):
     RetKLc(M-1) for M = 3..7 equals 0.0481 / 0.0321 / 0.0192 / 0.0107 / 0.0057;
     RetKLc(M) = 0 exactly.
  2. prop:rd-nonconvex: 5-state instance D(M) values and log-convexity failure;
     4-state remark instance.
  3. Csiszar representation lemma: the KL generator satisfies the (dagger)
     identity exactly; the reverse-KL generator does not (defect -0.1657).
  4. prop:lsyncu-binomial: the explicit M=4 witness machine has adaptive
     synchronization depth exactly 6 = C(4,2) and is Moore-minimal.
  5. thm:stream-lower-bound: forcing-stream simulation at L=2 forces exactly
     M*L = 8 mistakes with a consistent adversary machine.
  6. prop:bernoulli-fisher-scales: the ratio tends to 0.50009.
  7. prop:kl-simplex-sharp: RetKL(1) = 2 eps^2 + (4/3) eps^4 + O(eps^6).
  8. thm:esyncsi-theta: cyclic-shift machine depth == L for L = 1..6.
  9. thm:global-kl-simplex: minorant RetKL(phi) >= sum_{i>=M} lambda_i(Sigma_p)
     on 400 random instances (0 violations).
 10. remark on gating: chained-readout consistency check.

Usage:  python3 verify_numerical_claims.py   (requires numpy; ~30 s)
"""
